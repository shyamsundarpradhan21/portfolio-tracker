...
- **Interactive render-verify = certify.mjs (headless puppeteer), NOT the claude-in-chrome browser.** The automation Chrome can't hydrate the Next *dev* app (chunks load, React never hydrates → frozen "Loading portfolio…", no console logs, no `next` runtime); certify's puppeteer renders the *identical* server fine. Don't rabbit-hole killing the dev server / clearing `.next` to "fix" it — the user's own Chrome hydrates normally. Confirm visuals via certify + code; hand the user `localhost:3000` for anything hover/interaction-only.
...
- **"Did X get parsed/ingested?" → check `data/ingest-manifest.json` (the event log), NOT `inbox/`.** PASSed intakes are DELETED on success (raw docs never persisted), so a successfully-parsed note NEVER appears in `inbox/failed|unrecognized`. Searching inbox/ alone falsely reports "no such note". (Did exactly this: wrongly told the user yesterday's Dhan note didn't exist — it had already PASSed to `ledger:cn:11135427` earlier that day. The failing note I found in `inbox/failed/` was a *different*, coincidentally same-dated Upstox note.) Filter the manifest by `meta.broker` / `meta.date` / `ts`; a note re-arrives daily under the SAME generic filename, so dedup/identity is the `naturalKey` (contract-note no), never the filename.
