...
- **Payslip upload → net/CMPF/CMPS now AUTO-append** (`parse-payslip.py --write`, append-only — never overwrites a reconciled month — then the daemon reseeds). A NEW month whose net/CMPF/CMPS deviates >2× the trailing median is flagged `[!] arrear-like` and SKIPPED (the naive per-slip sum is wrong there, e.g. May-26 CMPS ₹132,947 vs ₹9,019 median) → hand-reconcile those from the ingest log. The savings-rate card divides by net (`PAYSLIPS`), so a still-missing (flagged) month leaves it `null` / hidden. (A byte-identical re-upload is correctly `[DUP]`-skipped — check the manifest before assuming an intake failed.)
