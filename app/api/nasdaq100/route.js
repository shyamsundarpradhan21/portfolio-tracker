// Nasdaq-100 heatmap feed for Pre-Market Insights (US view). Fetched lazily (only
// when the Pre-Market/Macro tab is open) since it pulls ~100 constituent quotes —
// too heavy to ride the 15-min whole-app price refresh. US mirror of /api/nifty50.
//
//   GET /api/nasdaq100
...
export async function GET() {
  const results = (await Promise.all(NASDAQ100.map(fetchOne))).filter((r) => r && r.pct != null);
