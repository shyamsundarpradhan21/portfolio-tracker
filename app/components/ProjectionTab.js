...
function ProjectionTab({ nw, loan = 0, fx, sleeves = [], onDrift, baseYear, invested0, snapshots, histSeries, dayGain = {}, sleeveBasis = {}, cmpsRetirement, cmpsPension = 0, cmpsService = null, cmpsVested = false, cmpsVestYear = null, dataReady = true, footer = null }) {
  ...
  return (
    <div className="card sec pjx">
      ...
      {/* #25 — the standalone "Contribution by sleeve" waffle + legend was removed: the
          per-sleeve contribution waffle already lives INSIDE each period card (the <Waffle>
          strip on every 1M/3M/6M/1Y/MAX tile), so a separate block + colour legend was
          redundant. The tile waffles carry the same mix, per window, with a hover breakdown. */}
